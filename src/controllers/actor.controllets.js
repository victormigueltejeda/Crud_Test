const pool = require("../db");

const getAllActor = async (req, res, next) => {
  try {
    const result = await pool.query("SELECT * FROM actores");
    res.json(result.rows);
  } catch (error) {
    next(error);
  }
};

const getActor = async (req, res, next) => {
  try {
    const { id } = req.params;
    const result = await pool.query("SELECT * FROM actores WHERE id = $1", [
      id,
    ]);

    if (result.rows.length === 0)
      return res.status(404).json({
        message: "Actor No Encontrada",
      });

    res.json(result.rows[0]);
  } catch (error) {
    next(error);
  }
};

const addActor = async (req, res, next) => {
  const { nombre_completo } = req.body;

  try {
    const result = await pool.query(
      "INSERT INTO actores(nombre_completo) VALUES ($1) RETURNING *",
      [nombre_completo]
    );

    res.json(result.rows[0]);
  } catch (error) {
    next(error);
  }
};

const deleteActor = async (req, res, next) => {
  const { id } = req.params;
  try {
    const result = await pool.query(
      "DELETE FROM actores WHERE id = $1 RETURNING *",
      [id]
    );

    if (result.rows.length === 0)
      return res.status(404).json({
        message: "Actor No Encontrado",
      });

    return res.sendStatus(204);
  } catch (error) {
    next(error);
  }
};

const updateActor = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { title, genero } = req.body;

    const result = await pool.query(
      "UPDATE actores SET title= $1, genero= $2 WHERE id= $3 RETURNING *",
      [title, genero, id]
    );

    if (result.rows.length === 0)
      return res.status(404).json({
        message: "Actor No Encontrado",
      });
    return res.json(result.rows[0]);
  } catch (error) {
    next(error);
  }
};

module.exports = {
  getAllActor,
  getActor,
  addActor,
  deleteActor,
  updateActor,
};
